...
const MovieList = ({ page, setPage, inputValue, setInputValue }) => {
  ...
  useEffect(async () => {
    if (view === 'soon-movies' || view === 'movies') {
      await getSoonMovies().then((data) => {
        setMovieList(data.results);
      });
    } else if (view === 'search-movies' && inputValue !== '') {
      await searchMovies(page, inputValue).then((data) => {
        setMovieList(data.results);
        window.scroll(0, 0);
      });
    } else {
      await getMovies(page).then((data) => {
        setMovieList(data.results);
        window.scroll(0, 0);
      });
    }
    ...
    <div className="MovieList-container">
      ...
      {view === 'search-movies' ? (
        <div className="MovieList-page">
          {page === 1 ? (
            <button type="button" id="MovieList-prev" disabled>
              Anterior
            </button>
          ) : (
            <button
          ...
          {page < 1000 ? (
            ...
            <button type="button" id="MovieList-next" disabled>
              Anterior
            </button>
          )}
        </div>
...
<img
  src="http://image.tmdb.org/t/p/w500/1fd66a0c216a49a2dfce.jpeg"
